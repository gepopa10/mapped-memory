...
private:
    ...
    size_t initial_window_size_bytes;
...
public:
    FileMappedRegion(const std::string &file, size_t initial_size)
        : filename(file), window_size_bytes(initial_size), initial_window_size_bytes(initial_size), file_mapping(nullptr), mapped_region(nullptr)
    {
    ...
    {
        mapped_region->flush();
        window_size_bytes += initial_window_size_bytes;
